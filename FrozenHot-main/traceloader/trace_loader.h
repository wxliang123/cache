...
std::vector<std::string> extract_suffix_string(std::string const& start,
...
class TraceLoader {
 public:
  enum Operation {
    ...
    /** store this data */
    set,
    /** store this data, only if data not already exist; if add fails, promote
       anyway */
    add,
  ...
  ~TraceLoader() {
    if (requests_ != nullptr) free(requests_);
    requests_ = nullptr;
  }
  ...
  TraceLoader(const uint64_t num, double zipf_const) {
    ...
    auto zipf_generator =
        new ScrambledZipfianGenerator(0, item_num, zipf_const);
  ...
  // MSR
  TraceLoader(const uint64_t num, std::string work_file,
    ...
    requests_ = (request*)malloc(100ull * 1024 * 1024 *
                                 1024);  // allocate 100GB (as much as possible)
    if (requests_ == NULL) {
    ...
    while (infile.good() && !infile.eof() && load_index_ < num_) {
      ...
      int end = (strtol(words[4].c_str(), NULL, 10) +
                 strtol(words[5].c_str(), NULL, 10) - 1) >>
                12;
      op = words[3];
      if (op == "Read") {
        ...
        for (int index = start; index <= end && load_index_ < num_; index++) {
          requests_[load_index_] =
              request(TraceLoader::READ, index);  // regarded as read
          load_index_++;
        }
      } else {
  ...
  TraceLoader(const uint64_t num, std::string work_file, std::string work_type,
              std::string workload_start, std::string workload_end,
              bool binary) {
    ...
    for (auto suffix : suffixs) {
      ...
      if (workload_start == "" || workload_end == "") {
        ...
        std::cout << "open file :" + work_file + "." + suffix + ".bin"
                  << std::endl;
      }
      if (!infile.good()) {
      ...
      // as request size = 8B, we can read 1024 requests each time; to save
      // execution time
      int request_buffer_size = 1024;
      char* request_buffer =
          (char*)malloc(infile_read_size * request_buffer_size);
      while (infile.good() && !infile.eof() && load_index_ < num_) {
        infile.read(request_buffer, infile_read_size * request_buffer_size);
        if (infile.eof()) {
          for (int i = 0; i < file_request_num % request_buffer_size; i++) {
            int op_bin =
                *(int*)(request_buffer + i * infile_read_size + sizeof(int));
            uint32_t hash_key =
                *(uint32_t*)(request_buffer + i * infile_read_size +
                             sizeof(int) * 2);
  ...
  TraceLoader(const uint64_t num, std::string work_file, std::string work_type,
    ...
    for (auto suffix : suffixs) {
      ...
      while (infile.good() && !infile.eof() && load_index_ < num_) {
        ...
        op = words[1];
        unsigned int hashed_string = string_hash(words[0]);
        ...
        load_index_++;
        if (load_index_ % 100000000 == 0) {
          auto end = std::chrono::high_resolution_clock::now();
          auto duration = std::chrono::duration<double>(end - start).count();
          start = end;
          printf("finished %ld (100m) in %.3lf s \n", load_index_ / 100000000,
                 duration);
          fflush(stdout);
        }
      }
  ...
  request GetRequest(key_size_t index) { return requests_[index % queue_size]; }
  uint64_t GetQueueSize() { return queue_size; }
