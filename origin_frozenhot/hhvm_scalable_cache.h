...
const double max_frozen_ratio = 0.9;
...
class ConcurrentScalableCache {
  using Shard = LRU_FHCache<TKey, TValue, THash>;
 ...
 public:
  ...
  virtual ~ConcurrentScalableCache() {}
...
ConcurrentScalableCache<TKey, TValue, THash>::ConcurrentScalableCache(
    size_t maxSize, size_t numShards, int rebuild_freq)
    : m_maxSize(maxSize),
      m_numShards(numShards),
      FROZEN_THRESHOLD(rebuild_freq) {
  if (m_numShards == 0) {
  ...
  for (size_t i = 0; i < m_numShards; i++) {
    ...
    m_shards.emplace_back(
        std::make_shared<LRU_FHCache<TKey, TValue, THash>>(s));
  }
}

template <class TKey, class TValue, class THash>
typename ConcurrentScalableCache<TKey, TValue, THash>::Shard&
ConcurrentScalableCache<TKey, TValue, THash>::getShard(const TKey& key) {
  THash hashObj;
...
template <class TKey, class TValue, class THash>
bool ConcurrentScalableCache<TKey, TValue, THash>::find(TValue& ac,
                                                        const TKey& key) {
  return getShard(key).find(ac, key);
}

template <class TKey, class TValue, class THash>
bool ConcurrentScalableCache<TKey, TValue, THash>::insert(const TKey& key,
                                                          const TValue& value) {
  return getShard(key).insert(key, value);
}

template <class TKey, class TValue, class THash>
void ConcurrentScalableCache<TKey, TValue, THash>::delete_key(const TKey& key) {
  getShard(key).delete_key(key);
}

template <class TKey, class TValue, class THash>
void ConcurrentScalableCache<TKey, TValue, THash>::snapshotKeys(
...
template <class TKey, class TValue, class THash>
size_t ConcurrentScalableCache<TKey, TValue, THash>::Size() {
  size_t size = 0;
  for (size_t i = 0; i < m_numShards; i++) {
...
template <class TKey, class TValue, class THash>
void ConcurrentScalableCache<TKey, TValue, THash>::PrintStat() {
...
template <class TKey, class TValue, class THash>
double ConcurrentScalableCache<TKey, TValue, THash>::PrintMissRatio() {
  size_t total_hit = 0, total_miss = 0;
  for (size_t i = 0; i < m_numShards; i++) {
    size_t hit_num = 0, miss_num = 0;
...
template <class TKey, class TValue, class THash>
void ConcurrentScalableCache<TKey, TValue, THash>::PrintFrozenStat() {
...
template <class TKey, class TValue, class THash>
void ConcurrentScalableCache<TKey, TValue, THash>::PrintGlobalLat() {
  uint64_t total_failed = 0, total_update = 0;
  for (size_t i = 0; i < m_numShards; i++) {
    total_failed += m_shards[i]->failed_count.load();
    total_update += m_shards[i]->update_count.load();
  }

  printf("Insert - total failed: %ld, total update: %ld\n", total_failed,
         total_update);
  ...
  printf("Total Avg Lat: %.3lf (size: %lu, miss ratio: %.6lf)\n",
         (avg_hit * hit_num + avg_other * other_num) / total_num, total_num,
         other_num * 1.0 / total_num);
  fflush(stdout);
...
template <class TKey, class TValue, class THash>
double ConcurrentScalableCache<TKey, TValue, THash>::PrintStepLat() {
  size_t hit_num = 0, other_num = 0;
  ...
  printf(
      ...
      "%.4lf)\n",
      temp, total_num,
      SSDLOGGING_TIME_DURATION(cursor, curr_time) / 1000 / 1000,
      other_num * 1.0 / total_num);
  cursor = curr_time;
  return temp;
}
...
template <class TKey, class TValue, class THash>
uint64_t ConcurrentScalableCache<TKey, TValue, THash>::GetStepSize() {
...
template <class TKey, class TValue, class THash>
double ConcurrentScalableCache<TKey, TValue, THash>::PrintStepLat(
    double& avg_hit, double& avg_other) {
  size_t hit_num = 0, other_num = 0;
  auto curr_time = SSDLOGGING_TIME_NOW;
  printf("- Hit ");
...
template <class TKey, class TValue, class THash>
void ConcurrentScalableCache<TKey, TValue, THash>::FastHashMonitor() {
...
WAIT_STABLE:
  ...
  while (!should_stop) {
    ...
    /* This logic is for the case that the cache is already full != cache is
     * stable still need several extra loops to evict the coldest items the
     * metric is miss ratio non-decreasing */
    if (last_size >= size) {              // cache is full
      if (last_miss_ratio <= miss_ratio)  // Miss ratio is non-decreasing
        wait_count++;
      if (wait_count >=
          WAIT_STABLE_THRESHOLD) {  // Waiting too long, stop filling process
  ...
  auto wait_stable_duration =
      SSDLOGGING_TIME_DURATION(start_wait_stable, SSDLOGGING_TIME_NOW);
  ...
  printf("\ndraw curve\ndata pass %lu\n", print_step_counter++);
  PrintMissRatio();
  /* become aware of DC latency (hit lat) and DC miss latency + disk latency
   * (miss lat) */
  ...
  printf(
      "\nHothash lat: %.3lf, 100%% Frozen Avg lat: %.3lf us, Miss Rate: "
      "%.3lf\n",
  ...
  for (size_t i = 0; i < container.size(); i++) {
    ...
    if (tSize < 0.01) {
      ...
      printf("when baseline, avg from %.3lf to %.3lf\n", avg,
             avg / (1 + FH_PERFORMANCE_THRESHOLD));
    ...
    // when tSize is large, we regard it as ~100% FC
      ...
      tSize = max_frozen_ratio;
      avg = tFC_hit_ * FC_lat + tMiss * (miss_lat + FC_lat) +
            (1 - tFC_hit_ - tMiss) * (FC_lat + DC_hit_lat);
    } else {
      avg = tFC_hit_ * FC_lat + tMiss * (miss_lat + FC_lat) +
            (1 - tFC_hit_ - tMiss) * (FC_lat + DC_hit_lat);
    }
    ...
    if (avg < best_avg) {
      ...
      printf(
          "(Update) best avg: %.3lf us, best size: %.3lf (w. FC_hit: %.3lf, "
          "miss: %.3lf)\n",
  ...
  auto learning_duration =
      SSDLOGGING_TIME_DURATION(start_learning_time, SSDLOGGING_TIME_NOW);
...
CONSTRUCT:
  ...
  double baseline_performance_with_threshold =
      baseline_performance_for_query / (1 + FH_PERFORMANCE_THRESHOLD);
  ...
  /*
   * 1. construct_container is used to store the shard id that we want to
   * construct FH
   * 2. fail_list is used to store the shard id that we failed to construct FH
   *    The reason to have fail_list is that we don't have to construct FH for
   * all shards We only construct FH for shards that can EASILY see improvement
   * (i.e. see within a few try)
   *    TODO @ Ziyue: do sensitivity analysis to explore more about this
   * threshold
   * 3. pass_count is used to count the number of pass
   * example: we have 4 shards, and we want to construct FH for shard 0, 1, 2,
   * 3, allow 2 pass Init: construct_container = {0, 1, 2, 3} fail_list = {}
   * pass_count = 0 < 2 1st pass: 0~2 success, 3 fail, construct_container = {3}
   * fail_list = {} pass_count = 1 2nd pass: 3 success, construct_container = {}
   * fail_list = {} pass_count = 2 end; or 2nd pass: 3 fail, construct_container
   * = {} fail_list = {3} pass_count = 2 end;
   */
  while (!should_stop && !construct_container.empty()) {
    ...
    for (size_t i = 0; i < pass_len; i++) {
      ...
      // deconstruct (fail)
      if (performance >
          baseline_metrics[shard_id] / (1 + FH_PERFORMANCE_THRESHOLD)) {
        construct_container.push(shard_id);
  ...
  auto construct_duration =
      SSDLOGGING_TIME_DURATION(start_construct_time, SSDLOGGING_TIME_NOW);
  ...
  while (!should_stop) {
    ...
    // TODO @ Ziyue: how to become 'optimal' instead of only better than
    // baseline
    auto delta = (baseline_performance_with_threshold - performance) /
                 baseline_performance_with_threshold * thput_step /
                 baseline_step;
    performance_depletion +=
        delta;  // INTEGRATION to help decide whether to stop FH
    ...
    if (total_step >
        construct_step * FROZEN_THRESHOLD) {  // Periodic Reconstruction of FH
      printf(
          "\nAfter %lu frozen step (> %d * %lu = %lu), need periodically "
          "refresh!\n",
          total_step, FROZEN_THRESHOLD, construct_step,
      ...
      /**
       * This is a round to check whether need to deconstruct since
       * performance_depletion is initialized to COUNT_THRESHOLD, if
       * performance_depletion > COUNT_THRESHOLD, it means FH still outperforms
       * baseline so we can continue to run FH, but need to reset
       * performance_depletion to COUNT_THRESHOLD or we will not be able to
       * beware of the performance degradation till deplete all benefits else
       * (performance_depletion <= COUNT_THRESHOLD), it means FH is not good
       * enough
       */
...
DECONSTRUCT:
  ...
  auto query_duration =
      SSDLOGGING_TIME_DURATION(start_query_stage, SSDLOGGING_TIME_NOW);
   ...
   * Q2: Isn't wait stable for cache warm up, and we already run the cache for
   * so long?
   * WAIT_STABLE is not just for warmup, can also be a recovery from FH for
   * example, after deconstruct, we need to wait for a while to let the cache to
   * recover itself, and then we can start to construct or the profiling results
   * (curve) might not be accurate.
   */
  if (!should_stop) {
...
template <class TKey, class TValue, class THash>
void ConcurrentScalableCache<TKey, TValue, THash>::monitor_stop() {
  should_stop = true;
  printf("\nshould stop = true\n");
}
