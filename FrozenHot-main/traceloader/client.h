...
class Client {
 public:
  Client(utils::Properties props) {
    ...
    if (!work_type.compare("Zipf")) {
      ...
      // binary format
      // loader_1 = new TraceLoader(request_num, param, work_type,
      // twitter_start, twitter_end, true); original format
      loader_1 = new TraceLoader(request_num, param, work_type, twitter_start,
                                 twitter_end);
    } else {
    ...
    if (!cache_type.compare("LRU")) {
      ...
      scalable_cache =
          new Cache(real_cache_size, seg, Cache::LRU_FH, rebuild_freq);
      is_fh_cache_type = true;
    } else if (!cache_type.compare("LFU_FH")) {
      scalable_cache =
          new Cache(real_cache_size, seg, Cache::LFU_FH, rebuild_freq);
      is_fh_cache_type = true;
    } else if (!cache_type.compare("FIFO_FH")) {
      scalable_cache =
          new Cache(real_cache_size, seg, Cache::FIFO_FH, rebuild_freq);
      is_fh_cache_type = true;
  ...
  ~Client() {
    if (loader_1 != nullptr) delete loader_1;
    if (scalable_cache != nullptr) delete scalable_cache;
  }
 ...
 public:
  ...
  typedef tstarling::ConcurrentScalableCache<size_t,
 ...
 public:
  ...
  void warmup() {
    ...
    printf("granularity: %lu and large granularity: %lu\n", Granularity,
           Large_Granularity);
#endif
    fn = std::bind(&Client::worker_thread, this, std::placeholders::_1,
                   std::placeholders::_2, std::placeholders::_3,
                   std::placeholders::_4, std::placeholders::_5);
#ifdef FH_ENABLE
    fn_monitor =
        std::bind(&Client::FastHashMonitor, this, std::placeholders::_1);
#endif
...
#else
      if (base_coreid + i > 39)
        threads.emplace_back(fn, single_num, base_coreid + i + 40, true, i == 0,
                             start_);
      else
        threads.emplace_back(fn, single_num, base_coreid + i, true, i == 0,
                             start_);
#endif
    }
#ifdef FH_ENABLE
    ...
    auto work_duration =
        SSDLOGGING_TIME_DURATION(start_work, SSDLOGGING_TIME_NOW);
#ifdef FH_ENABLE
  ...
  void worker_thread(uint64_t num, int coreid, bool is_warmup, bool is_master,
    ...
    for (uint64_t i = 0; i < num; i++) {
      ...
      if ((!scalable_cache->stop_sample_stat && i % Granularity == 0) ||
          (scalable_cache->stop_sample_stat && i % Large_Granularity == 0))
      ...
      if (!scalable_cache->stop_sample_stat && (i % Granularity == 0)) {
        ...
        tstarling::req_latency_[(hashObj.hash(key) >> shift) % seg].insert(
            duration);
      ...
      } else if (scalable_cache->stop_sample_stat &&
                 (i % Large_Granularity == 0)) {
        auto duration =
            SSDLOGGING_TIME_DURATION(start_of_loop, SSDLOGGING_TIME_NOW);
        if (ret_flag)
          tstarling::total_hit_latency_.insert(duration);
