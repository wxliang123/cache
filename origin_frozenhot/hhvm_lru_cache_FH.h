...
#include "FHCache.h"
#include "fast_hash/clht_hash.h"

// time
#define SPDK_TIME (__builtin_ia32_rdtsc())
#define SPDK_TIME_DURATION(start, end, tsc_rate) \
  (((end) - (start)) * 1.0 / (tsc_rate) * 1000 * 1000)

#define SSDLOGGING_TIME_NOW (std::chrono::high_resolution_clock::now())
#define SSDLOGGING_TIME_DURATION(start, end) \
...
constexpr static uint64_t MARKER_KEY = 2;
constexpr static uint64_t TOMB_KEY = 1;
constexpr static uint64_t SPECIAL_KEY = 0;
 ...
 *
 * The find() operation fills a ConstAccessor object, which is a smart
 * pointer similar to TBB's const_accessor. After eviction, destruction of
 * the value is deferred until all ConstAccessor objects are destroyed.
 *
 * The implementation is generally conservative, relying on the documented
 * behaviour of tbb::concurrent_hash_map. LRU list transactions are
 * protected with a single mutex. Having our own doubly-linked list
 * implementation helps to ensure that list transactions are sufficiently
 * brief, consisting of only a few loads and stores. User code is not
 * executed while the lock is held.
 *
 * The acquisition of the list mutex during find() is non-blocking
 * (try_lock), so under heavy lookup load, the container will not stall,
 * instead some LRU update operations will be omitted.
 *
...
class LRU_FHCache : public FHCache<TKey, TValue, THash> {
  using BaseCache = FHCache<TKey, TValue, THash>;
 ...
 public:
  ...
  void thread_init(int tid) override { m_fasthash->thread_init(tid); }
 ...
 private:
  ...
  std::unique_ptr<fast_hash::CLHT_Hash<TValue>> m_fasthash;
...
// printf("LOCK_P is %.4lf\n", LOCK_P);
  ...
  m_fasthash.reset(new fast_hash::CLHT_Hash<TValue>(0, align_len));
}
...
bool LRU_FHCache<TKey, TValue, THash>::construct_ratio(double FC_ratio) {
  ...
  while (temp_node != &m_fast_tail) {
    ...
    if (!m_map.find(temp_hashAccessor, temp_node->m_key)) {
      ...
      // delete_temp->m_key, count); printf("insert count: %lu v.s. eviction
      // count: %lu\n", count, eviction_count);
      temp_node = temp_node->m_next;
      if (delete_temp->isInList()) delink(delete_temp);
      delete delete_temp;
      fail_count++;
      continue;
    }
...
void LRU_FHCache<TKey, TValue, THash>::deconstruct() {
  ...
  if (fast_hash_ready) fast_hash_ready = false;
  lock.unlock();
...
template <class TKey, class TValue, class THash>
bool LRU_FHCache<TKey, TValue, THash>::find(TValue& ac, const TKey& key) {
...
bool LRU_FHCache<TKey, TValue, THash>::get_curve(bool& should_stop) {
  ...
  for (int i = 0; i < 45 && !should_stop; i++) {
    ...
    LRU_FHCache::curve_container.insert(
        LRU_FHCache::curve_container.end(),
        curve_data_node(FC_ratio, FC_hit, miss));
  }
  BaseCache::sample_flag = true;

  printf("\ncurve container size: %lu\n", BaseCache::curve_container.size());
...
bool LRU_FHCache<TKey, TValue, THash>::insert(const TKey& key,
  ...
  if (!m_map.insert(hashAccessor, hashMapValue)) {
    ...
    // LRU_FHCache::update_count++;
    return false;
  }
  if (fast_hash_construct) eviction_counter++;
...
template <class TKey, class TValue, class THash>
void LRU_FHCache<TKey, TValue, THash>::snapshotKeys(std::vector<TKey>& keys) {
...
template <class TKey, class TValue, class THash>
inline void LRU_FHCache<TKey, TValue, THash>::pushAfter(ListNode* nodeBefore,
                                                        ListNode* nodeAfter) {
...
void LRU_FHCache<TKey, TValue, THash>::delete_key(const TKey& key) {
  ...
  } else if ((tier_ready || fast_hash_ready) && m_fasthash->find(key, ac) &&
             (ac != nullptr)) {
    node->m_key = TOMB_KEY;
    std::unique_lock<ListMutex> lock(m_listMutex);
    // m_fasthash.set_key_value(key, nullptr);
  ...
  if (m_map.erase(hashAccessor)) m_size--;
}
#endif
