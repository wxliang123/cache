...
// Segment cache implementation. This class is not thread-safe
...
struct EntryHandle {
  ...
  // Beginning of the key (MUST BE THE LAST FIELED IN THIS STRUCT!)
  char key_data[1];
...
class SegmentCacheShard final : public CacheShardBase {
 ...
 public:  // Function definitions expected as parameter to ShardedCache
  ...
  EntryHandle* CreateStandalone(const Slice& key, uint32_t hash,
                                Cache::ObjectPtr obj,
                                const Cache::CacheItemHelper* helper,
...
class SegmentCache final : public ShardedCache<SegmentCacheShard> {
 public:
  explicit SegmentCache(const SegmentCacheOptions& opts);
